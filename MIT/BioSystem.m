...
classdef BioSystem < handle
    ...
    properties
        ...
        symbols = ['t'];    %#ok<NBRAK> % list of all the Compositor symbols
                            % - by initializing symbols with t we allow t to be 
                            %   a variable that's not a Compositor or Constant
    ...
    methods
        ...
        function new_compositor = AddCompositor(self, varargin)
            ...
            self.symbols = [self.symbols; new_compositor.symb];
        end
        ...
        function [] = determine_rates(self)
            if ~self.rates_determined
                ...
                for k = 1:length(self.compositors)
                    ...
                    % their values
                    constant_syms = arrayfun(@(c){c.symb}, self.constants);
                    constant_vals = arrayfun(@(c){c.value}, self.constants);
                    symexpr = subs(self.compositors(k).rate, ...
                                constant_syms, ...
                                constant_vals);
        ...
        function [T, Y] = run(self, tspan, varargin)
            ...
            if isempty(varargin)
                ...
                [ T, Y ] = ode23s(@self.sys_ode, tspan, y0, odeset(varargin{:}));
            end
